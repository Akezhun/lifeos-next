...
import { Archive, BarChart3, Bell, CalendarDays, Import, NotebookPen, Settings, ShieldCheck, Sparkles } from "lucide-react";
...
const modules = [
  ...
  { href: "/notifications", title: "Notifications", desc: "Email/worker settings and test endpoints.", icon: Bell },
...
export default function Home() {
  return <AppShell title="LifeOS 2.0" subtitle="V12.1 Feature Parity Rebuild: новая среда, но функции Classic возвращены как рабочие модули."><AuthGate>{(user) => <HomeInner user={user} />}</AuthGate></AppShell>;
}
...
function HomeInner({ user }: { user: any }) {
  ...
  return <div className="space-y-6">
    <div className="life-card-strong p-5 md:p-7">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-3 max-w-3xl text-white/58">Supabase is the database. GitHub Vault will become an Obsidian mirror later. For now: local testing, no GitHub push until this feels right.</p>
        </div>
        <div className="grid grid-cols-2 gap-3 text-center md:grid-cols-3">
          <Metric label="Trackers" value={counts.trackers || 0}/><Metric label="Journal entries" value={counts.journal_entries || 0}/><Metric label="Schedule tokens" value={(counts.schedule_tokens || 0)+(counts.schedule_rules || 0)}/><Metric label="Events" value={counts.tracker_events || 0}/><Metric label="Archived" value={counts.archived || 0}/><Metric label="Version" value="12.1"/>
        </div>
      </div>
    </div>
    ...
    <div className="life-card p-5"><h3 className="mb-2 flex items-center gap-2 text-lg font-black"><Archive size={18}/> V12.1 rule</h3><p className="text-white/58">Эту версию не пушим в GitHub, пока она не догонит Classic по ощущениям и функциям. Проверяем локально через <code>npm run dev</code>.</p></div>
  </div>;
}
